...
Window {
        ...
        Column {
            ...
            Button {
                ...
                MouseArea {
                    ...
                    cursorShape: Qt.PointingHandCursor
                    onClicked: {
                        Qt.openUrlExternally("https://console.firebase.google.com/u/0/project/aslr-asu/authentication/users");
                    }
                }
            }
        }
    ...
    Item {
        ...
        FileDialog {
            ...
            onAccepted: {
                var selected = mapDialog.selectedFile
                mapImage.source = selected
                mmapImage.source = selected
            ...
            function loadFromCsv(filePath) {
                ...
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === XMLHttpRequest.DONE && (xhr.status === 200 || xhr.status === 0)) {
                        coordinateLoader.parseCSVAndSetGoals(xhr.responseText);
                    }
                }
                xhr.send();
            }

            function parseCSVAndSetGoals(csvContent) {
                ...
                coordSettings.setValue("Coord_Keys", coordKeys.join(";"));

                cameraMapRow.goalCoordinates = newGoals;
                cameraMapRow.shelfKeys = Object.keys(newGoals).filter(n => n.startsWith("Shelf"));
    ...
    Item {
        ...
        Column {
            ...
            Column {
                ...
                ComboBox {
                    ...
                    width: 180
                    height: 40
                    model: cameraMapRow.locationKeys
        ...
        Component.onCompleted: {
            // Load saved map
            var lastMap = settings.value("LastMap", "")
            if (lastMap !== "") {
                mapImage.source = lastMap
                mmapImage.source = lastMap
                console.log("Loaded saved map:", lastMap)
            } else {
                console.log("No saved map found.")
            }

            // Load saved coordinates
            var keysString = coordSettings.value("Coord_Keys", "");
            if (keysString !== "") {
                var keyList = keysString.split(";");
                var allGoals = {};
                for (var i = 0; i < keyList.length; i++) {
                    var name = keyList[i];
                    var json = coordSettings.value("Coord_" + name, "");
                    if (json !== "") {
                        var obj = JSON.parse(json);
                        allGoals[name] = obj;
                        console.log("Restored coordinate:", name, "→", obj.x, obj.y);
                    }
                }

                cameraMapRow.goalCoordinates = allGoals;
                cameraMapRow.shelfKeys = Object.keys(allGoals).filter(n => n.startsWith("Shelf"));
                cameraMapRow.locationKeys = Object.keys(allGoals).filter(n => n.startsWith("Location"));
                shelfDropdown.model = cameraMapRow.shelfKeys;
                locationDropdown.model = cameraMapRow.locationKeys;
            } else {
        ...
        Row {
            ...
            property var shelfKeys: []
            property var locationKeys: []
            ...
            property string currentMap: settings.lastMap
            ...
            Rectangle {
                ...
                Image {
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        onReleased: {
                            cameraMapRow.robotX = parent.x + parent.width / 2;
                            cameraMapRow.robotY = parent.y + parent.height / 2;
                            console.log("Robot manually moved to:", cameraMapRow.robotX, cameraMapRow.robotY);
                        }
                    }
                }
